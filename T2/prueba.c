...
int vendo(int precio, char vendedor, char comprador) {
    ...
    if ((exisVendedor == 0)  (precioGlob  precio)){
        ...
        printf(precio remplazado, precio actual es %i, lo vende %sn, precioGlob,vendedor);
    ...
    pthread_cond_wait(&c,&m);
    printf(desperto el vendedor %sn,vendedor);

    if(isVendido == 1){
        strcpy(comprador,compradorGlob);
        printf(fue comprado por %in,precio);
        precioGlob = 0;
        isVendido = 0;
        exisVendedor = 0;
        pthread_mutex_unlock(&m);
...
int compro(char comprador, char vendedor) {
    ...
    printf(inicio comprador %s n,comprador);
    if (exisVendedor == 1){
        strcpy(vendedor,vendedorGlob);
        compradorGlob = comprador;
        int pricePay = precioGlob;
        isVendido = 1;
        printf(precio pagado por %s %in,comprador , pricePay);
        pthread_cond_broadcast(&c);
        pthread_mutex_unlock(&m);
        return pricePay;
    }
    printf(no hay nada a la venta cuando el comprado %s, paso n,comprador);
    pthread_mutex_unlock(&m);
    return 0;
